...
class Ion_auth
{
    ...
    public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
    {
        ...
        $persona_data = ['nombres' => $additional_data['first_name'], 'apellidos' => $additional_data['last_name'], 'cedula' => '0999999999', 'fotografia' => $additional_data['fotografia'],'user_id'=>$id,'estado'=>1];

        $this->persona_model->registrar($persona_data);
    ...
    public function have_permision($check_page)
    {
        $user_id = $this->session->userdata('user_id');
        ...
        $this->db_autenticacion->where('menus.slug', $check_page);
     ...
     * @author Mathew
     **/
    public function register_nuevo($identity, $password, $email, $additional_data = array(), $groups, $cedula)
    {
        $this->ion_auth_model->trigger_events('pre_account_creation');

        $email_activation = $this->config->item('email_activation', 'ion_auth');

        $id = $this->ion_auth_model->register_nuevo($identity, $password, $email, $additional_data, $groups);

        $persona_data = ['nombres' => $additional_data['first_name'], 'apellidos' => $additional_data['last_name'], 'cedula' => $cedula, 'fotografia' => $additional_data['fotografia'], 'user_id' => $id, 'estado' => 1];
