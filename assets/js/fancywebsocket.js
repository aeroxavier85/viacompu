		...
		if(message == null || message == "")//aqui es donde se realiza toda la accion
			...
			else
			{
				...
				switch(JSONdata[0].actualizacion)//que tipo de actualizacion vamos a hacer(un nuevo mensaje, solicitud de amistad nueva, etc )
				{
					...
					break;
					case '2':
					actualiza_contador_mensaje(message);
					break;
...
$(document).ready(function() 
{
	...
	Server = new FancyWebSocket('ws://192.168.0.139:12345');
...
function actualiza_contador_mensaje(message)
{
    var JSONdata    = JSON.parse(message); //parseo la informacion
   // var tipo = JSONdata[0].tipo;
    var contador_mensaje = JSONdata[0].mensaje;
    //var fecha = JSONdata[0].fecha;

    //var contenidoDiv  = $("#contador_mensaje").html();
    //var mensajehtml   = mensaje;
    $("#contador_mensaje").html('');
    if(contador_mensaje>0){
        var mensaje=' <a href="#" onclick="actualizar_leidas()">'+contador_mensaje+'</a>';
    }else{
        var mensaje=contador_mensaje;
    }
    $("#contador_mensaje").html('Mensaje: '+mensaje);
}
